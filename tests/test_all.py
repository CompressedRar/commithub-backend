...
from models.Tasks import Main_Task
...
class TestDepartmentModel:
    ...
    def test_department_creation(self, test_department):
        ...
        assert test_department.description == "Engineering Department"
...
class TestUserRoutes:
    ...
    def test_get_user_profile_protected(self, client):
        """Test profile endpoint requires auth."""
        response = client.get('/api/v1/users/profile')
        assert response.status_code in [401, 403]
    
    def test_get_user_profile_with_auth(self, client, auth_headers):
        """Test getting profile with auth."""
...
class TestDepartmentRoutes:
    ...
    def test_get_all_departments(self, client):
        """Test getting all departments."""
        response = client.get('/api/v1/department')
        assert response.status_code == 200
        assert isinstance(response.json, (list, dict))
    
    def test_get_department_by_id(self, client, test_department):
        """Test getting single department."""
        response = client.get(f'/api/v1/department/{test_department.id}')
        assert response.status_code in [200, 404]
    
    def test_create_department_requires_auth(self, client):
        """Test department creation requires auth."""
        response = client.post('/api/v1/department',
            json={'name': 'New Dept', 'description': 'Test'}
        )
        assert response.status_code in [401, 403, 400]
    
    def test_create_department_with_auth(self, client, auth_headers):
        """Test creating department."""
        response = client.post('/api/v1/department',
            json={'name': 'IT Department', 'description': 'IT'},
            headers=auth_headers
        )
        assert response.status_code in [200, 201, 400, 403]
    
    def test_update_department(self, client, auth_headers, test_department):
        """Test updating department."""
        response = client.patch(f'/api/v1/department/{test_department.id}',
            json={'name': 'Updated Dept'},
            headers=auth_headers
        )
        assert response.status_code in [200, 400, 403, 404]
    
    def test_department_performance_report(self, client, auth_headers, test_department):
...
class TestSettingsRoutes:
    ...
    def test_update_settings_requires_auth(self, client):
        """Test settings update requires auth."""
        response = client.patch('/api/v1/settings',
            json={'quantity_formula': '(actual/target)*5'}
        )
        assert response.status_code in [401, 403]
    
    def test_update_settings_with_auth(self, client, auth_headers):
    ...
    def test_reset_period_requires_auth(self, client):
        """Test period reset requires auth."""
        response = client.patch('/api/v1/settings/reset', json={})
        assert response.status_code in [401, 403]
    
    def test_reset_period_with_auth(self, client, auth_headers, system_settings):
...
class TestPCRRoutes:
    ...
    def test_create_opcr_requires_auth(self, client):
        """Test OPCR creation requires auth."""
        response = client.post('/api/v1/pcr/opcr',
            json={'department_id': 1, 'ipcr_ids': [1, 2]}
        )
        assert response.status_code in [401, 403, 400]
    
    def test_create_opcr_with_auth(self, client, auth_headers, test_department):
        """Test creating OPCR."""
        response = client.post('/api/v1/pcr/opcr',
    ...
    def test_get_opcr(self, client, auth_headers):
        """Test getting OPCR."""
        response = client.get('/api/v1/pcr/opcr/1', headers=auth_headers)
        assert response.status_code in [200, 404, 401]
    
    def test_create_ipcr_requires_auth(self, client):
        """Test IPCR creation requires auth."""
        response = client.post('/api/v1/pcr/ipcr',
            json={'main_task_ids': [1, 2]}
        )
        assert response.status_code in [401, 403, 400]
    
    def test_create_ipcr_with_auth(self, client, auth_headers):
        """Test creating IPCR."""
        response = client.post('/api/v1/pcr/ipcr',
            json={'main_task_ids': []},
            headers=auth_headers
    ...
    def test_get_ipcr(self, client, auth_headers):
        """Test getting IPCR."""
        response = client.get('/api/v1/pcr/ipcr/1', headers=auth_headers)
        assert response.status_code in [200, 404, 401]
...
class TestLogRoutes:
    ...
    def test_get_logs_with_auth(self, client, auth_headers):
        """Test getting logs."""
        response = client.get('/api/v1/logs', headers=auth_headers)
        assert response.status_code in [200, 401, 403]
    
    def test_filter_logs_by_action(self, client, auth_headers):
        """Test filtering logs."""
        response = client.get('/api/v1/logs?action=LOGIN', headers=auth_headers)
        assert response.status_code in [200, 401, 403]
...
class TestIntegrationFlow:
    ...
    def test_department_workflow(self, client, auth_headers, test_department):

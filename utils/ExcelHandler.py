...
def createNewWeightedOPCR(data, assigned, admin_data):
    datee = datetime.datetime.now().month
    year = str(datetime.datetime.now().year)
    period = ""
    if datee <= 6:
    ...
    wb = load_workbook("excels/WeightedOPCRTest.xlsx")
    ws = wb.active
    ...
    for i in data:
        ...
        for g, h in i.items():
            ...
            for a in h:
                ...
                # dito ilagy yung weights

                prepareCells(ws, str("D"+str(row)), str("D"+str(row + 5)))
                ws["D"+str(row)] = str(a["description"]["task_weight"] * 100) + "%" #weights dito 
                ws["D"+str(row)].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                ws["D"+str(row)].font = Font(bold=True)

                prepareCells(ws, str("E"+str(row)), str("E"+str(row + 1)))
                ws["E"+str(row)] = a["summary"]["target"] #quabntity dito
                ws["E"+str(row)].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                ...
                #description
                prepareCells(ws, str("F"+str(row)), str("G"+str(row + 1)))
                ws["F"+str(row)] = a["description"]["target"] # desc quant 
                ws["F"+str(row)].alignment = Alignment(wrap_text=True, horizontal="left", vertical = "center")
                ws.row_dimensions[row].height = 45
                ...
                prepareCells(ws, str("F"+str(row+4)), str("G"+str(row + 5)))
                ws["F"+str(row+4)] = a["description"]["alterations"] # desc quant
                ws[f"F{row+4}"].alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
    ...
    weighted_average = f"=SUM(R{startingrow}: R{endrow})"
    weighted_averagecell = str("R"+str(row))
    prepareCells(ws, str("R"+str(row)), str("R"+str(row+1)))
    ws["R"+str(row)] = f"=SUM(R{startingrow+1}: R{endrow})"
    ws["R"+str(row)].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ws["R"+str(row)].number_format = numberformat
    ws["R"+str(row)].font = Font(bold=True)
    ...
    prepareCells(ws, str("R"+str(row)), str("R"+str(row+1)))
    ws["R"+str(row)] = f'=IF(AND({total_weighted}>=0, {total_weighted}<=1.49), "POOR", IF(AND({total_weighted}>=1.5, {total_weighted}<=2.49), "UNSATISFACTORY", IF(AND({total_weighted}>=2.5, {total_weighted}<=3.49), "SATISFACTORY", IF(AND({total_weighted}>=3.5, {total_weighted}<=4.49), "VERY SATISFACTORY", IF(AND({total_weighted}>=4.5, {total_weighted}<=5), "OUTSTANDING")))))'
    ws["R"+str(row)].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ws["R"+str(row)].number_format = numberformat
    ws["R"+str(row)].font = Font(bold=True)
    ...
    for i in data:
        ...
        for g, h in i.items():
            ...
            for a in h:
                ...
                if a["description"]["timeliness_mode"] == "deadline":
                    prepareCells(ws, f"J{row+2}", f"J{row+3}")
                    ws[f"J{row+2}"] = abs(round(a["working_days"]["actual"] )) if a["working_days"]["actual"] != 0 else 0 # desc quant
                    ws[f"J{row+2}"].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                    ws[f"J{row+2}"].font = Font(bold=True)
                else:
                    prepareCells(ws, f"J{row+2}", f"J{row+3}")
                    ws[f"J{row+2}"] =  abs(round(a["working_days"]["actual"])) if a["working_days"]["actual"] != 0 else 0
                    ws[f"J{row+2}"].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                    ws[f"J{row+2}"].font = Font(bold=True)
                
                prepareCells(ws, str("J"+str(row+4)), str("J"+str(row + 5)))
                ws["J"+str(row+4)] = round(a["corrections"]["actual"]) if a["corrections"]["actual"] != 0 else 0 # desc quant
                ws["J"+str(row+4)].alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
                ws["J"+str(row+4)].font = Font(bold=True)
    ...
    for i in data:
        ...
        for g, h in i.items():
            ...
            for a in h:
                ...
                ws["E"+str(row+2)] = a["description"]["time"] + "/s " # desc quant
                ...
                ws["J"+str(row+2)] = abs(round(a["working_days"]["actual"] / a["frequency"])) if a["working_days"]["actual"] != 0 else 0 # desc quant
